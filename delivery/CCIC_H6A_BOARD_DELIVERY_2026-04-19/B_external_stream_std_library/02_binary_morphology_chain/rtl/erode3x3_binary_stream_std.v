...
// 对每个有效窗口拍执行 3x3 二值腐蚀。
module erode3x3_binary_stream_std #(
    ...
    output reg  [MAX_LANES*DATA_W-1:0]     m_data,  // 腐蚀后的输出数据，lane0 放在最低位。
    ...
    function [DATA_W-1:0] erode9;
        ...
        begin
            ...
            erode9 = all_set ? {DATA_W{1'b1}} : {DATA_W{1'b0}};
        end
    endfunction
                ...
                for (lane_idx = 0; lane_idx < MAX_LANES; lane_idx = lane_idx + 1) begin
                    if (s_keep[lane_idx]) begin
                        m_data[lane_idx*DATA_W +: DATA_W] <=
                            erode9(s_data[lane_idx*DATA_W*9 +: DATA_W*9]);
                    end else begin
