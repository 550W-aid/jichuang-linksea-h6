...
module tb_gray_threshold_erode_small_std;
...
function [7:0] expected_erode;
    ...
    begin
        ...
        expected_erode = all_set ? 8'hFF : 8'h00;
    end
endfunction
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        expected_final = expected_erode(final_idx);
        if ((final_keep !== 1'b1) ||
...
initial begin
    ...
    if (mask_mismatch_count != 0) begin
        $fatal(1, "tb_gray_threshold_erode_small_std threshold mismatch count=%0d first=%0d",
               mask_mismatch_count, mask_first_mismatch_idx);
    end
    if (final_mismatch_count != 0) begin
        $fatal(1, "tb_gray_threshold_erode_small_std erode mismatch count=%0d first=%0d",
               final_mismatch_count, final_first_mismatch_idx);
    end
    $display("tb_gray_threshold_erode_small_std passed.");
    $finish;
end
