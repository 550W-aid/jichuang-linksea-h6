...
module tb_gray_window_sobel_std;
...
function [7:0] abs8_sat;
    ...
    begin
        ...
        if (abs_val > 13'd255) begin
            abs8_sat = 8'hFF;
        end else begin
            abs8_sat = abs_val[7:0];
        end
    end
endfunction
...
function [7:0] sobel9;
    ...
    begin
        ...
        mag = abs8_sat(gx) + abs8_sat(gy);
        if (mag > 14'd255) begin
            sobel9 = 8'hFF;
        end else begin
            sobel9 = mag[7:0];
        end
...
initial begin
    ...
    file_gray = $fopen("sim_gray_window_sobel_gray.hex", "w");
    file_window = $fopen("sim_gray_window_sobel_window.hex", "w");
    file_final = $fopen("sim_gray_window_sobel_final.hex", "w");
    if ((file_gray == 0) || (file_window == 0) || (file_final == 0)) begin
        $error("tb_gray_window_sobel_std failed to open output files.");
        $finish;
    end
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        if (timeout_count > TIMEOUT_CYCLES) begin
            $error("tb_gray_window_sobel_std timeout.");
            $finish;
        end
    end
end
...
initial begin
    ...
    if (final_mismatch_count != 0) begin
        $display("sobel first mismatch index = %0d", final_first_mismatch_idx);
        $fatal(1, "sobel mismatch count = %0d", final_mismatch_count);
    end

    $display("tb_gray_window_sobel_std passed with %0d pixels.", PIXELS);
    $finish;
end
