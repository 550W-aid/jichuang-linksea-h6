...
module tb_bilinear_resize_realtime_stream_std_smoke;
...
always @(posedge clk or negedge rst_n) begin
    if (!rst_n) begin
        ...
        expected_rgb_q = expected_rgb(out_idx);
        if ((m_keep !== 1'b1) ||
            (m_data !== expected_rgb_q) ||
            (m_sof !== (out_idx == 0)) ||
            (m_eol !== ((out_idx % OUT_WIDTH) == (OUT_WIDTH - 1))) ||
            (m_eof !== (out_idx == (OUT_PIXELS - 1)))) begin
...
initial begin
    ...
    while (out_idx < OUT_PIXELS) begin
        ...
        if (timeout_cycles > 300) begin
            $fatal(1, "tb_bilinear_resize_realtime_stream_std_smoke timeout");
        end
    end

    #20;
    if (mismatch_count != 0) begin
        $fatal(1, "tb_bilinear_resize_realtime_stream_std_smoke mismatch count=%0d first=%0d",
               mismatch_count, first_mismatch_idx);
    end
    $display("tb_bilinear_resize_realtime_stream_std_smoke passed.");
    $finish;
end
