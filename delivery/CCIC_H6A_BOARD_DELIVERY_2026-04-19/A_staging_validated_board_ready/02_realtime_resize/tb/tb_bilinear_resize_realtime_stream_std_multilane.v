...
module tb_bilinear_resize_realtime_stream_std_multilane;
...
task send_frame;
    begin
        ...
        send_beat(pixel_mem[2],  pixel_mem[3],  2'b11, 1'b0, 1'b1, 1'b0);
        send_beat(pixel_mem[4],  pixel_mem[5],  2'b11, 1'b0, 1'b0, 1'b0);
        send_beat(pixel_mem[6],  pixel_mem[7],  2'b11, 1'b0, 1'b1, 1'b0);
        send_beat(pixel_mem[8],  pixel_mem[9],  2'b11, 1'b0, 1'b0, 1'b0);
        send_beat(pixel_mem[10], pixel_mem[11], 2'b11, 1'b0, 1'b1, 1'b0);
        send_beat(pixel_mem[12], pixel_mem[13], 2'b11, 1'b0, 1'b0, 1'b0);
        send_beat(pixel_mem[14], pixel_mem[15], 2'b11, 1'b0, 1'b1, 1'b1);
    end
endtask
...
initial begin
    ...
    while (out_count < 4) begin
        ...
        if (wait_cycles > 200) begin
            $fatal(1, "Timed out waiting for all multilane output beats.");
        end
    end

    $display("tb_bilinear_resize_realtime_stream_std_multilane passed.");
    $finish;
end
...
always @(posedge clk) begin
    if (m_valid && m_ready) begin
        case (out_count)
            0: begin
                if (m_data[23:0] !== expected_rgb(0,0) || m_data[47:24] !== 24'h000000 || m_keep !== 2'b01 || {m_sof,m_eol,m_eof} !== 3'b100) begin
                    $fatal(1, "Unexpected beat0 data/meta");
                end
            end
            1: begin
                if (m_data[23:0] !== expected_rgb(1,0) || m_data[47:24] !== expected_rgb(2,0) || m_keep !== 2'b11 || {m_sof,m_eol,m_eof} !== 3'b010) begin
                    $fatal(1, "Unexpected beat1 data/meta");
                end
            end
            2: begin
                if (m_data[23:0] !== expected_rgb(0,1) || m_data[47:24] !== 24'h000000 || m_keep !== 2'b01 || {m_sof,m_eol,m_eof} !== 3'b000) begin
                    $fatal(1, "Unexpected beat2 data/meta");
                end
            end
            3: begin
                if (m_data[23:0] !== expected_rgb(1,1) || m_data[47:24] !== expected_rgb(2,1) || m_keep !== 2'b11 || {m_sof,m_eol,m_eof} !== 3'b011) begin
                    $fatal(1, "Unexpected beat3 data/meta");
                end
            end
            default: begin
                $fatal(1, "Unexpected extra multilane output beat count=%0d", out_count);
            end
        endcase
