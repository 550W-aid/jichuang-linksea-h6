...
module bilinear_resize_realtime_stream_std #(
    ...
    // Two line buffers: one stores the previous row, the other stores the current row.
    // The current signoff result is based on LUTRAM inference for these row buffers.
    (* ram_style = "distributed" *) reg [23:0] line_mem0 [0:IMG_WIDTH-1];
    (* ram_style = "distributed" *) reg [23:0] line_mem1 [0:IMG_WIDTH-1];
    ...
    always @* begin
        ...
        if (sample_valid_q &&
            (out_x_q < active_out_width_q) &&
            (out_y_q < active_out_height_q) &&
            (sample_x_q == curr_src_x1_w) &&
            (sample_y_q == curr_src_y1_w)) begin

            emit_valid_w = 1'b1;
            emit_p11_w   = sample_rgb_q;
            emit_p10_w   = (curr_src_x0_w == sample_x_q) ? sample_rgb_q : sample_curr_left_q;

            if (curr_src_y0_w == sample_y_q) begin
                emit_p01_w = sample_rgb_q;
                emit_p00_w = (curr_src_x0_w == sample_x_q) ? sample_rgb_q : sample_curr_left_q;
            end else begin
                emit_p01_w = sample_prev_row_q;
                emit_p00_w = (curr_src_x0_w == sample_x_q) ? sample_prev_row_q : sample_prev_left_q;
            end

            emit_sof_w = (out_x_q == 0) && (out_y_q == 0);
            emit_eol_w = (out_x_q == (active_out_width_q - 1'b1));
            emit_eof_w = emit_eol_w && (out_y_q == (active_out_height_q - 1'b1));

            if (out_x_q == (active_out_width_q - 1'b1)) begin
                next_out_x_w    = 16'd0;
                next_out_y_w    = out_y_q + 16'd1;
                next_src_x_fp_w = 32'd0;
                next_src_y_fp_w = src_y_fp_q + active_scale_y_fp_q;
            end else begin
                next_out_x_w    = out_x_q + 16'd1;
                next_out_y_w    = out_y_q;
                next_src_x_fp_w = src_x_fp_q + active_scale_x_fp_q;
                next_src_y_fp_w = src_y_fp_q;
            end
        end
    end
        ...
        if (!rst_n) begin
            ...
            if (pipe_ce) begin
                ...
                if (s_fire) begin
                    ...
                    sample_valid_q <= 1'b1;
                    sample_x_q <= input_x_w;
                    sample_y_q <= input_y_w;
                    sample_rgb_q <= in_pixel_w;
                    sample_curr_left_q <= s_sof ? 24'd0 : curr_left_q;
                    sample_prev_left_q <= s_sof ? 24'd0 : prev_row_left_q;

                    if (write_bank_q) begin
                        sample_prev_row_q <= line_mem0[input_x_w];
                        line_mem1[input_x_w] <= in_pixel_w;
                        prev_row_left_q <= s_sof ? 24'd0 : line_mem0[input_x_w];
                    end else begin
                        sample_prev_row_q <= line_mem1[input_x_w];
                        line_mem0[input_x_w] <= in_pixel_w;
                        prev_row_left_q <= s_sof ? 24'd0 : line_mem1[input_x_w];
                    end
