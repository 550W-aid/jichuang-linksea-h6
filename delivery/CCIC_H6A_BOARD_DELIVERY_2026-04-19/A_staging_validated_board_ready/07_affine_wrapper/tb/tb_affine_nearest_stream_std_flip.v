...
module tb_affine_nearest_stream_std_flip;
    ...
    begin
        ...
        while (!m_valid) begin
            ...
            if (wait_cycles > 64) begin
                $fatal(1, "Timed out waiting for affine flip output.");
            end
        end
    end
endtask
...
initial begin
    ...
    push_cfg(-16'sd256, 16'sd0, 16'sd3,
             16'sd0,    16'sd256, 16'sd0);
    send_demo_frame();

    if ((active_m00 !== -16'sd256) || (active_m02 !== 16'sd3)) begin
        $fatal(1, "Horizontal-flip matrix did not commit on frame start.");
    end

    expect_output_pixel(24'h000004, 1'b1, 1'b0, 1'b0);
    expect_output_pixel(24'h000003, 1'b0, 1'b0, 1'b0);
    expect_output_pixel(24'h000002, 1'b0, 1'b0, 1'b0);
    expect_output_pixel(24'h000001, 1'b0, 1'b1, 1'b0);
    ...
    expect_output_pixel(24'h000005, 1'b0, 1'b1, 1'b0);
    ...
    expect_output_pixel(24'h000009, 1'b0, 1'b1, 1'b0);
    ...
    expect_output_pixel(24'h00000D, 1'b0, 1'b1, 1'b1);

    $display("tb_affine_nearest_stream_std_flip passed.");
    $finish;
end
