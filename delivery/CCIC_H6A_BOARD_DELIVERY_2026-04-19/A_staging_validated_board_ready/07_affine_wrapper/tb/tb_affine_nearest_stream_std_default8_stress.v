...
module tb_affine_nearest_stream_std_default8_stress;
    ...
    begin
        ...
        while (!m_valid) begin
            ...
            if (wait_cycles > 128) begin
                $fatal(1, "Timed out waiting for affine stress output.");
            end
        end
    end
endtask
...
task expect_output_beat;
    ...
    begin
        ...
        repeat (stall_cycles) begin
            @(posedge clk);
            if (!m_valid) begin
                $fatal(1, "Output lost valid during backpressure.");
            end
        end
...
task build_expected_translate;
    ...
    begin
        ...
        for (lane_idx = 0; lane_idx < valid_count; lane_idx = lane_idx + 1) begin
            expected_keep[lane_idx] = 1'b1;
            src_x = start_x + lane_idx + dx;
            src_y = (row_base / IMG_WIDTH) + dy;
            if ((src_x >= 0) && (src_x < IMG_WIDTH) &&
                (src_y >= 0) && (src_y < IMG_HEIGHT)) begin
                expected_data[lane_idx*PIX_W +: PIX_W] =
                    demo_pixels[src_y*IMG_WIDTH + src_x];
            end else begin
...
initial begin
    for (pix_idx = 0; pix_idx < PIXELS; pix_idx = pix_idx + 1) begin
        demo_pixels[pix_idx] = 24'h340000 + (pix_idx * 5);
    end
    ...
    build_expected_translate(0, 0, 8, 1, 1);
    expect_output_beat(expected_data, expected_keep, 1'b1, 1'b0, 1'b0, 0);
    build_expected_translate(0, 8, 2, 1, 1);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b1, 1'b0, 2);
    build_expected_translate(10, 0, 8, 1, 1);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b0, 1'b0, 0);
    build_expected_translate(10, 8, 2, 1, 1);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b1, 1'b1, 1);
    @(posedge clk);
    if ((active_m02 !== 16'sd1) || (active_m12 !== 16'sd1)) begin
        $fatal(1, "Affine translation did not commit on the first frame.");
    end

    push_cfg(16'sd256, 16'sd0, 16'sd0,
             16'sd0,   16'sd256, 16'sd0);
    send_frame();

    build_expected_identity(0, 8);
    expect_output_beat(expected_data, expected_keep, 1'b1, 1'b0, 1'b0, 1);
    build_expected_identity(8, 2);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b1, 1'b0, 0);
    build_expected_identity(10, 8);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b0, 1'b0, 0);
    build_expected_identity(18, 2);
    expect_output_beat(expected_data, expected_keep, 1'b0, 1'b1, 1'b1, 2);
    @(posedge clk);
    if ((active_m02 !== 16'sd0) || (active_m12 !== 16'sd0)) begin
        $fatal(1, "Queued identity did not commit on the second frame.");
    end

    $display("tb_affine_nearest_stream_std_default8_stress passed.");
    $finish;
end
