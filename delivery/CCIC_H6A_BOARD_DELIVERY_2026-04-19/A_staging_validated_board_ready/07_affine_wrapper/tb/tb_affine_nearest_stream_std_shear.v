...
module tb_affine_nearest_stream_std_shear;
    ...
    begin
        ...
        while (!m_valid) begin
            ...
            if (wait_cycles > 64) begin
                $fatal(1, "Timed out waiting for affine shear output.");
            end
        end
    end
endtask
...
initial begin
    ...
    push_cfg(16'sd256, 16'sd256, 16'sd0,
             16'sd0,   16'sd256, 16'sd0);
    send_demo_frame();

    if ((active_m00 !== 16'sd256) || (active_m01 !== 16'sd256)) begin
        $fatal(1, "Shear matrix did not commit on frame start.");
    end
    ...
    $display("tb_affine_nearest_stream_std_shear passed.");
    $finish;
end
