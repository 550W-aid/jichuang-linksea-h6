...
// 将单像素流转换为带边界复制的 3x3 窗口流。
// 核心思路是：
// 1. 用两条行缓存保存“前两行同列像素”
// 2. 用 3 组横向移位寄存器保存“当前三行最近两列像素”
// 3. 当前拍新像素到来时，三者一起拼成一个完整 3x3 窗口
// 因此，即便上游是“1 个 clk 只进 1 个像素”，这里也能连续产出窗口流。
// 当前实现按 MAX_LANES = 1 使用。
module window3x3_stream_std #(
    ...
    input  wire [MAX_LANES*DATA_W-1:0]     s_data,  // 输入像素数据，lane0 放在最低位。
    ...
    // 下游标准流接口输出。
    output reg                             m_valid, // 当前拍窗口输出有效。
    input  wire                            m_ready, // 下游当前拍可以接收输出。
    output reg  [MAX_LANES*DATA_W*9-1:0]   m_data,  // 3x3 窗口数据，lane0 放在最低位。
    ...
    // 状态定义：
    // ST_REAL       : 正常接收真实输入像素。
    // ST_EOL_FLUSH  : 当前行结束后，补输出最右侧复制窗口。
    // ST_BOTTOM_ROW : 最后一行结束后，逐列补输出底边复制窗口。
    // ST_BOTTOM_EOL : 输出右下角最终复制窗口，然后回到初始状态。
    ...
    // 两条行缓存用于保存前两行像素，供 3x3 窗口拼接。
    // Keep the line buffers reset-free so synthesis can map them into block RAM.
    (* ramstyle = "M4K" *) reg [DATA_W-1:0] line_mem0 [0:IMG_WIDTH-1];
    (* ramstyle = "M4K" *) reg [DATA_W-1:0] line_mem1 [0:IMG_WIDTH-1];
